...
describe('Agent Block Registry', () => {
  it('registers every configurable agent', () => {
    ...
    expect(names).toContain('chapters.summarize')
  })
...
describe('Model Role Registry — fallback chain derivation', () => {
  ...
  it('lets chapter agents inherit the librarian model before generation', () => {
    const chain = modelRoleRegistry.getFallbackChain('chapters.summarize')
    expect(chain).toEqual(['chapters.summarize', 'chapters', 'librarian', 'generation'])
  })

  it('registers namespace-level roles only', () => {
    const roles = modelRoleRegistry.list()
    const keys = roles.map(r => r.key).sort()
    expect(keys).toEqual(['chapters', 'character-chat', 'directions', 'generation', 'librarian', 'story-setup'])
  })
})
