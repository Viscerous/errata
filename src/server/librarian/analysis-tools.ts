...
export const reportPassageInputSchema = z.object({
  ...
  endedEntries: LiveStateEndingsInputSchema.default([]),
  // A question is a thread: it becomes the stored ThreadOperation.label (max
  // 240), so the grammar bound stays at or below that. Named as questions
  // because a small model fills a list called "threads" with topics.
  openQuestions: forgivingArray(z.string().trim().max(240), { max: 6 }).default([])
    .describe('Questions the story has raised and not yet answered that stand in the foreground after this passage, each phrased as a question. Repeat an existing one exactly to keep it open; a new question opens a new one. This is a snapshot: omitted existing questions become dormant.'),
  answeredQuestions: forgivingArray(z.string().trim().max(240), { max: 4 }).default([])
    .describe('Existing questions this passage answers or closes for good, repeated exactly. Use empty [] if none.'),
 ...
 * is addressed by its label, the only form the analyst is shown; anything else
 * opens a new one. Live
...
function normalizeThreads(
  ...
  const existing = (value: string) => byLabel.get(normalizeContinuityKey(value))
  ...
  for (const value of openQuestions) {
...
function normalizeContinuityProjection(
  input: Pick<ReportPassageInput, 'scene' | 'present' | 'characters' | 'entities' | 'endedEntries' | 'openQuestions' | 'answeredQuestions'>,
  segments: TextSegment[],
  ...
  const threads = normalizeThreads(input.openQuestions ?? [], input.answeredQuestions ?? [], registry.thread, skipped)
  const liveStates = normalizeLiveStateReports(
    { present: input.present, characters: input.characters, entities: input.entities, endedEntries: input.endedEntries },
    registry.items,
    checkedFragments,
  )
...
export function createAnalysisTools(
  ...
  const resolveCorrectionOperations = async (corrections: Array<z.infer<typeof correctionProposalItemSchema>>) => {
    ...
    for (const correction of corrections) {
      ...
      // Evidence opens a correction as it opens maintenance: a new name in the
      // prose licenses a new record, never a rewrite of one nothing contradicts.
      if (!collector.contradictions.some((contradiction) => contradiction.fragmentIds.includes(correction.fragmentId))) {
        unresolved.push({
          operationId: '',
          action: 'replace_text',
          reason: `No contradiction in this passage cites ${correction.fragmentId}; a correction only repairs a record the prose contradicts.`,
        })
        continue
      }
