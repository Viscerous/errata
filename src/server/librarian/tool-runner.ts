import { Output, ToolLoopAgent, asSchema, jsonSchema, stepCountIs, streamText, type ToolSet } from 'ai'
...
export interface ToolLoopPassArgs {
  ...
  /** 'required' when the pass has nothing to say except through its tools. */
 ...
 * prompt therefore carries what a tool declaration would have: the tool's
 * description and its schema with field descriptions.
 */
export function structuredOutputForm(tool: ToolSet[string]): string {
