...
The Agents panel is in the sidebar under **Management**. Pick an agent to open its block editor. It shows model settings, agent-specific toggles, tool toggles, and a unified list of prompt blocks (builtin + custom), merged and sorted by role then order. Its context preview begins with a payload map: exact compiled-message and serialized-tool proportions, the largest declared block/tool sources, and the combined request estimate. When the selected backend advertises a context window, the map also shows estimated prompt occupancy and the approximate space left for output. This is diagnostic only: an advertised window does not introduce an automatic rejection or trimming policy. Each serialized tool schema remains inspectable below it. For agents that answer in several requests, such as Librarian Analyze, each request is a selectable view: the passage report, and the conditional record-maintenance request. No request is presented as though it receives every tool.
