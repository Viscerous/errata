...
- **Debugging "what did the librarian change in this fragment's history"**: check `librarian/storage.ts`'s analyses, not traces — `versions[]` on the fragment itself also shows the `reason` stamp (for example `'auto-apply'` or `'manual-accept'`).
