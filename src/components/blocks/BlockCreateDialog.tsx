    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[480px] max-h-[85vh] flex flex-col overflow-hidden" data-component-id="block-create-dialog">
        ...
        <div className="flex-1 overflow-y-auto space-y-5 py-1 pr-1">
          {/* Name */}
          <div>
            <h4 className="text-ui-label text-muted-foreground uppercase tracking-[0.15em] font-medium mb-2">
              Name
            </h4>
            <Input
              value={name}
            ...
            <div>
              ...
              <div className="flex rounded-lg bg-muted/25 p-[3px] gap-[3px]">
                ...
                <button type="button"
                  key={value}
                  className={cn(
                    'flex-1 flex items-center justify-center gap-1.5 px-2 py-[6px] rounded-md text-ui-label font-medium transition-all duration-150',
                    role === value
          ...
          <div>
            ...
            {type === 'script' && (
              <div className="mb-2.5 rounded-md bg-amber-500/5 border border-amber-500/10 px-3 py-2">
                <p className="text-ui-label text-amber-600/70 dark:text-amber-400/70 leading-relaxed">
                  Write a JS function body that returns a string. Access story data via <code className="font-mono bg-amber-500/10 px-1 rounded text-ui-label">ctx</code>: ctx.story, ctx.proseFragments, ctx.authorInput, etc.
                </p>
              </div>
            )}
