    ...
    <div className="flex flex-col h-full" data-cuelume-surface="bloom" data-component-id="debug-panel-root">
      ...
      <div className="flex items-center justify-between px-6 py-4 border-b border-border/50" data-component-id="debug-panel-header">
        <div className="flex items-center gap-2">
          ...
          <span className="text-ui-label text-muted-foreground uppercase tracking-wider">Generation Logs</span>
        </div>
        <Button size="icon" variant="ghost" className="size-7 text-muted-foreground" onClick={onClose} aria-label="Close debug panel" data-component-id="debug-close">
          <X className="size-4" />
        </Button>
      </div>
        ...
        {!directLookup && (
          <div className="w-56 border-r border-border/50 flex flex-col" data-component-id="debug-log-list">
            <div className="px-3 py-2.5 border-b border-border/50">
              <span className="text-ui-label font-medium text-muted-foreground uppercase tracking-wider">Recent</span>
            </div>
          ...
          {selectedLog ? (
            <>
              ...
              <div className="flex items-center gap-1 px-4 py-2.5 border-b border-border/50">
                {([
                  ...
                  >
                    ...
                    {tab === 'tools' && selectedLog.toolCalls.length > 0 && (
                      <Badge variant="secondary" className="ml-1 text-ui-label px-1 h-4">
                ...
                {/* Stats */}
                <div className="ml-auto flex items-center gap-2 text-ui-label text-muted-foreground">
                  {selectedLog.prewriterModel && (
                    <Badge variant="outline" className="text-ui-label h-4 px-1 font-normal border-primary/20 text-primary/60">
                      prewriter
                    </Badge>
                  )}
                  ...
                  {selectedLog.stepsExceeded && (
                    <Badge variant="destructive" className="text-ui-label h-4">EXCEEDED</Badge>
                  )}
                  {selectedLog.commitStatus === 'rejected' && (
                    <Badge variant="destructive" className="text-ui-label h-4">REJECTED</Badge>
                  )}
                </div>
              </div>
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-1.5 mt-1 text-muted-foreground">
        ...
        {log.toolCallCount > 0 && (
          <Badge variant="secondary" className="text-ui-label h-4 px-1">{log.toolCallCount} tools</Badge>
        )}
        {log.stepsExceeded && (
          <Badge variant="destructive" className="text-ui-label h-4 px-1">exceeded</Badge>
        )}
      </div>
    </button>
  )
...
function CopyButton({ text, className }: { text: string; className?: string }) {
  ...
  return (
    <button type="button"
      onClick={handleCopy}
      className={cn(
        'inline-flex items-center gap-1 text-ui-label text-muted-foreground hover:text-muted-foreground transition-colors',
        className,
      )}
  ...
  return (
    <div className="space-y-2">
      ...
      <div className="flex items-center gap-2 mb-1">
        ...
        <button type="button"
          onClick={toggleAll}
          className="ml-auto text-ui-label text-muted-foreground hover:text-muted-foreground transition-colors"
        >
      ...
      {log.toolCalls.map((tc, i) => {
        ...
        return (
          ...
          >
            ...
            >
              {/* Step number */}
              <span className="text-ui-label tabular-nums text-muted-foreground w-3 text-right shrink-0">
                {i + 1}
              </span>
              ...
              <div className="flex items-center gap-1 min-w-0 overflow-hidden">
                {argEntries.slice(0, 3).map(([key, val]) => (
                  <span
                    key={key}
                    className="text-ui-label font-mono text-muted-foreground bg-muted/20 rounded px-1 py-px truncate max-w-[140px] shrink-0"
                  >
                ...
                {argEntries.length > 3 && (
                  <span className="text-ui-label text-muted-foreground">+{argEntries.length - 3}</span>
                )}
              </div>
              ...
              <Badge
                variant="outline"
                className={cn(
                  'text-ui-label h-4 px-1 font-normal border-transparent shrink-0 ml-auto',
                  kind === 'write'
            ...
            {!isOpen && (
              <div className="px-3 py-1.5 flex items-center gap-1.5">
                <span className="text-ui-label text-muted-foreground shrink-0">
                  &rarr;
                </span>
                <span
                  className={cn(
                    'text-ui-label font-mono truncate',
            ...
            {isOpen && (
              <div className="divide-y divide-border/10">
                ...
                <div className="p-3">
                  ...
                  {argEntries.length === 0 ? (
                    <span className="text-ui-label text-muted-foreground italic">none</span>
                  ) : (
                    <div className="space-y-px">
                      {argEntries.map(([key, val]) => {
                        ...
                        return (
                          <div key={key} className="flex gap-2 text-ui-label font-mono leading-relaxed">
                ...
                <div className="p-3">
                  ...
                  <pre className="whitespace-pre-wrap text-ui-label font-mono text-muted-foreground max-h-[300px] overflow-y-auto leading-relaxed">
                    {resultStr}
                  </pre>
                </div>
              </div>
      ...
      {log.prewriterReasoning && (
        <div className="rounded-lg border border-primary/15 overflow-hidden">
          ...
          >
            ...
            <span className="text-ui-label font-medium text-primary/70">
              Prewriter Reasoning
            </span>
            <span className="text-ui-label text-muted-foreground tabular-nums ml-auto shrink-0">
          ...
          {prewriterReasoningExpanded && (
            <div className="p-3 max-h-[300px] overflow-y-auto">
              <pre className="whitespace-pre-wrap text-ui-label font-mono text-muted-foreground italic leading-relaxed">
                {log.prewriterReasoning}
              </pre>
            </div>
          )}
      ...
      {log.prewriterBrief && (
        <div className="rounded-lg border border-primary/15 overflow-hidden">
          ...
          >
            ...
            {log.prewriterModel && (
              <span className="text-ui-label text-muted-foreground font-mono">
                {log.prewriterModel}
              </span>
            )}
            <span className="text-ui-label text-muted-foreground tabular-nums ml-auto shrink-0 flex items-center gap-2">
          ...
          {prewriterExpanded && (
            <div className="p-3 max-h-[400px] overflow-y-auto">
              ...
              <div className="text-ui-label text-muted-foreground leading-relaxed prose prose-sm prose-muted max-w-none [&_p]:text-ui-label [&_p]:text-muted-foreground [&_p]:leading-relaxed">
